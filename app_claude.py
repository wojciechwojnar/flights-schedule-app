...
# Set page config
st.set_page_config(
    page_title="Flight Roster to Calendar Converter", page_icon="✈️", layout="wide"
)
            ...
            with pdfplumber.open(tmp_path) as pdf:
                ...
                for page in pdf.pages:
                    try:
                        ...
                        st.warning(
                            f"Could not extract text from page {page.page_number}: {e}"
                        )
                        continue
        finally:
            try:
...
class RosterParser:
    ...
    def extract_flights_from_sections(cls, sections: List[List[str]]) -> List[str]:
        ...
        for section in sections:
            for entry in section:
                if (
                    cls.WORKDAY_PATTERN.match(entry.split("LO")[0] + "LO")
                    if "LO" in entry
                    else False
                ):
    ...
    def parse_flight_to_event(
        cls, flight_line: str, period_start: datetime, period_end: datetime
...
class CalendarGenerator:
    ...
    def create_ics_from_events(events: List[FlightEvent], cutoff_date: datetime) -> str:
        ...
        for i, flight in enumerate(events):
            ...
            use_period_end = i > 0 and events[i - 1].day_of_month > flight.day_of_month
...
def extract_events_from_pdf(pdf_file) -> List[FlightEvent]:
    ...
    try:
        ...
        for flight_line in flight_lines:
            event = RosterParser.parse_flight_to_event(
                flight_line, period_start, period_end
            )
            if event:
                events.append(event)
        ...
        if st.button("🚀 Process PDF", type="primary"):
            with st.spinner("Processing PDF..."):
                ...
                if events:
                    ...
                    st.error(
                        "❌ No flights found in the PDF. Please check if the file format is correct."
                    )
    else:
        with status_placeholder.container():
