...
def extract_events_from_pdf(pdf_path):
    ...
    sections = []
    for line in lines[3:]:
        match_workday_start_date = re.match(r"^(\d{1,2})\.\s([A-Za-z]{3})", line)
    ...
    for section in sections:
        for entry in section:
            match_flight = re.match(r"^(\d{1,2})\.\s([A-Za-z]{3})\sLO\s(\d{1,5})", entry)
            if match_flight:
                flights.append(entry)

    events = []
    for flight in flights:
        match_flight = re.match(r"^(\d{1,2})\.\s([A-Za-z]{3})\sLO\s(\d{1,5})\s([A-Za-z]{3})\s(\d{4})\s(\d{4})\s([A-Za-z]{3})", flight)
        if match_flight:
            event = {
...
def create_ics_file(events, output_path):
    ...
    for event_data in events:
        ...
        event.description = f"Tracking: https://www.flightradar24.com/LO{event_data["flight_no"]}"
        if (prev_event_data is not None) and (prev_event_data["flight_day_of_month"] > event_data["flight_day_of_month"]):
